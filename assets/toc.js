/**
 * @fileOverView toc.js
 * @author zhangmeng on 15/3/30
 * borrowed from https://github.com/yijian166/md-toc.js
 */

(function(window){
    function Toc(root, id, options){
        this.el = root.shadowRoot.getElementById(id);
        //this.el = document.getElementById(id);
        if(!this.el) return;
        this.root = root;
        this.options = options || {};
        this.tocLevel = parseInt(this.options.level) || 0;
        this.tocClass = this.options['class'] || 'toc';
        this.tocTop = parseInt(this.options.top) || 0;
    ...
    Toc.prototype._showToc = function() {
        ...
        if(!this.options.targetId){
            ...
            this.root.getElementById(this.options.targetId).appendChild(this.toc);
        }
        ...
        if(this.tocTop > -1){
            window.onscroll = function(){
                ...
                if(t< self.tocTop){
                    self.toc.setAttribute('style','position:absolute;top:'+ self.tocTop +'px; right:0; width:180px');
                }else{
                    self.toc.setAttribute('style','position:fixed;top:10px; right:0;width:180px');
                }

            }
        }

        [].forEach.call(this.el.querySelectorAll('.toc a'), function (item) {
            item.addEventListener('click', function(e) {
                e.preventDefault();
                var curLink = e.target;
                var href = curLink.getAttribute('href');
                var targetContent = self.el.querySelector(href);
